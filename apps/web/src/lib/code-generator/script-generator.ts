import type { GeneratedApp } from "./code-brain";
import { extractTarget, parseIntent } from "./code-brain";
import {
  generatePythonFastAPIBackend,
  generateGoBackend,
  generateRustBackend,
} from "./multi-language-templates";
...
function extractTaskDescription(input: string): string {
  ...
  if (target && target !== 'application') {
    const cleaned = target
      ...
      .replace(/^(?:a\s+)?(?:script|program|tool|cli|utility)\s*/i, '')
      .trim();
...
function generatePythonScript(taskDescription: string, input: string): GeneratedApp {
  ...
  const baseTemplate = generatePythonFastAPIBackend(projectName, features);

  const mainPy = `#!/usr/bin/env python3
"""${projectName} - ${taskDescription}

Generated by AutoCoder (adapted from ${baseTemplate.framework} template)
"""
...
function generateGoScript(taskDescription: string, input: string): GeneratedApp {
  const projectName = toProjectName(taskDescription);
  const moduleName = projectName;
  const funcName = toFunctionName(taskDescription);

  const baseTemplate = generateGoBackend(projectName);
  const baseGoMod = baseTemplate.files.find(f => f.path === 'go.mod');
  const goVersion = baseGoMod ? (baseGoMod.content.match(/go (\d+\.\d+)/)?.[1] || '1.21') : '1.21';
...
// Adapted from ${baseTemplate.framework} backend template patterns.
func ${funcName}() error {
...
function generateRustScript(taskDescription: string, input: string): GeneratedApp {
  const projectName = toProjectName(taskDescription);
  const crateName = projectName.replace(/-/g, '_');
  const funcName = toSnakeCase(taskDescription);

  const baseTemplate = generateRustBackend(projectName);
  const baseCargoToml = baseTemplate.files.find(f => f.path === 'Cargo.toml');
  const edition = baseCargoToml ? (baseCargoToml.content.match(/edition = "(\d+)"/)?.[1] || '2021') : '2021';
...
function generateNodeScript(taskDescription: string, _input: string): GeneratedApp {
  const projectName = toProjectName(taskDescription);
  const funcName = toFunctionName(taskDescription);
...
function generateTypeScriptScript(taskDescription: string, _input: string): GeneratedApp {
  const projectName = toProjectName(taskDescription);
  const funcName = toFunctionName(taskDescription);
...
export function generateStandaloneScript(language: ScriptLanguage, taskDescription: string, input: string): GeneratedApp {
  switch (language) {
    case 'python':
      return generatePythonScript(taskDescription, input);
    case 'go':
      return generateGoScript(taskDescription, input);
    case 'rust':
      return generateRustScript(taskDescription, input);
    case 'typescript':
      return generateTypeScriptScript(taskDescription, input);
    case 'node':
      return generateNodeScript(taskDescription, input);
  }
}

const LANGUAGE_RUN_COMMANDS: Record<ScriptLanguage, string[]> = {
  python: [
  ...
  typescript: [
    ...
    'Or build and run: `npm run build && npm run run:built`',
  ],
  node: [
    'Install dependencies: `npm install`',
...
export function formatScriptResponse(app: GeneratedApp, language: ScriptLanguage): string {
  ...
  response += `## How to Run\n\n`;
  const commands = LANGUAGE_RUN_COMMANDS[language];
  for (let i = 0; i < commands.length; i++) {
    response += `${i + 1}. ${commands[i]}\n`;
  }
