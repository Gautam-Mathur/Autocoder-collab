...
export const repoImports = pgTable("repo_imports", {
  ...
  indexedAt: timestamp("indexed_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
  fileCount: integer("file_count").default(0),
  symbolCount: integer("symbol_count").default(0),
  framework: text("framework"),
  language: text("language"),
  repoIndex: jsonb("repo_index"),
});

export const debugRuns = pgTable("debug_runs", {
  id: serial("id").primaryKey(),
  conversationId: integer("conversation_id").references(() => conversations.id, { onDelete: "cascade" }),
  sandboxTier: text("sandbox_tier").notNull(),
  success: boolean("success").notNull().default(false),
  iterations: integer("iterations").default(0),
  finalErrors: text("final_errors").array(),
  durationMs: integer("duration_ms"),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
});

export const deploymentBundles = pgTable("deployment_bundles", {
  id: serial("id").primaryKey(),
  conversationId: integer("conversation_id").references(() => conversations.id, { onDelete: "cascade" }),
...
// Insert schemas for new tables
export const insertRepoImportSchema = createInsertSchema(repoImports).omit({ id: true, indexedAt: true });
export const insertDebugRunSchema = createInsertSchema(debugRuns).omit({ id: true, createdAt: true });
export const insertDeploymentBundleSchema = createInsertSchema(deploymentBundles).omit({ id: true, createdAt: true });

export type RepoImport = typeof repoImports.$inferSelect;
export type DebugRun = typeof debugRuns.$inferSelect;
export type DeploymentBundle = typeof deploymentBundles.$inferSelect;
